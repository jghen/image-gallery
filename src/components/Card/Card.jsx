import "./Card.css";
import Container from "../../hoc/Container.jsx";
import { useSelector } from "react-redux";
import { selectIsSignedIn } from "../../state/authSlice";
import { useState } from "react";

const Card = ({ id, imageUrl, title, subtitle, onCardClick, index }) => {
  const loggedIn = useSelector(selectIsSignedIn);
  const [loaded, setLoaded] = useState(false);

  const handleCardClick = (e) => {
    onCardClick(e);
  };
  console.log("loggedIn", loggedIn);

  const imageLoaded = (e) => {
    if (e.target.complete && e.target.height > 165) {
      setLoaded(true);
    }
  };

  const SkeletonText = () => {
    return <div className="skeleton-text" style={{ display: loaded ? "none" : "block" }} ></div>;
  };

  const loadedStyles = { 
    display: loaded ? "block" : "none" 
  }

  
  return (
    <section className="card" id={`card-${id}`} onClick={handleCardClick}>
      {loggedIn === true && (
        <button id="card-close-btn" className="btn" style={{ display: loaded ? "flex" : "none" }}>
          <span>+</span>
        </button>
      )}
      <Container>
        <div className="card-body">
          <div
            className="skeleton-img"
            style={{ display: loaded ? "none" : "block" }}
          ></div>
          <img
            style={loadedStyles}
            src={imageUrl}
            alt={title}
            loading={index > 6 ? "lazy" : "eager"}
            className="card-img"
            id={id}
            onLoad={imageLoaded}
          />
          {loaded ? <h3 id="card-title">{title}</h3> : <SkeletonText />}
          {loaded ? <p>{subtitle}</p> : <><SkeletonText /><SkeletonText /></>}
        </div>
      </Container>
    </section>
  );
};

export default Card;
